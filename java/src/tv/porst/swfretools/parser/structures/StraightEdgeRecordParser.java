...
public class StraightEdgeRecordParser {
	...
	public static StraightEdgeRecord parse(final BinaryParser parser) {
		...
		final SBits deltaX = generalLineFlag.value() || !vertLineFlag.value() ? parser.readSBits(numBits.value() + 2) : null;
		final SBits deltaY = generalLineFlag.value() || vertLineFlag.value() ? parser.readSBits(numBits.value() + 2) : null;
